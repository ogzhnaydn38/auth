...
const { dbCreateGrouppermission } = require("dbLayer");

class CreateGrouppermissionManager extends GivenPermissionManager {
  ...
  async doBusiness() {
    // Call DbFunction
    // make an awaited call to the dbCreateGrouppermission function to create the grouppermission and return the result to the controller
    const grouppermission = await dbCreateGrouppermission(this);
...
*/
  ...
  async raiseEvent() {
    GrouppermissionCreatedPublisher.Publish(this.output, this.session).catch(
      (err) => {
...
module.exports = CreateGrouppermissionManager;
