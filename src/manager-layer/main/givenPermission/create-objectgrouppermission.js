...
const { dbCreateObjectgrouppermission } = require("dbLayer");

class CreateObjectgrouppermissionManager extends GivenPermissionManager {
  constructor(request, controllerType) {
    super(request, {
      name: "createObjectgrouppermission",
  ...
  async doBusiness() {
    // Call DbFunction
    // make an awaited call to the dbCreateObjectgrouppermission function to create the objectgrouppermission and return the result to the controller
    const objectgrouppermission = await dbCreateObjectgrouppermission(this);

    return objectgrouppermission;
  }
...
*/
  ...
  async raiseEvent() {
    ObjectgrouppermissionCreatedPublisher.Publish(
      this.output,
      this.session,
    ).catch((err) => {
...
module.exports = CreateObjectgrouppermissionManager;
