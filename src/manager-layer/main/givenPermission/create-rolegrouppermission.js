...
const { dbCreateRolegrouppermission } = require("dbLayer");

class CreateRolegrouppermissionManager extends GivenPermissionManager {
  constructor(request, controllerType) {
    super(request, {
      name: "createRolegrouppermission",
  ...
  async doBusiness() {
    // Call DbFunction
    // make an awaited call to the dbCreateRolegrouppermission function to create the rolegrouppermission and return the result to the controller
    const rolegrouppermission = await dbCreateRolegrouppermission(this);
...
*/
  ...
  async raiseEvent() {
    RolegrouppermissionCreatedPublisher.Publish(
      this.output,
      this.session,
    ).catch((err) => {
...
module.exports = CreateRolegrouppermissionManager;
