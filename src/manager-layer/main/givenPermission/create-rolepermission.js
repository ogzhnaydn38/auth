...
const { dbCreateRolepermission } = require("dbLayer");

class CreateRolepermissionManager extends GivenPermissionManager {
  ...
  async doBusiness() {
    // Call DbFunction
    // make an awaited call to the dbCreateRolepermission function to create the rolepermission and return the result to the controller
    const rolepermission = await dbCreateRolepermission(this);
...
*/
  ...
  async raiseEvent() {
    RolepermissionCreatedPublisher.Publish(this.output, this.session).catch(
      (err) => {
...
module.exports = CreateRolepermissionManager;
