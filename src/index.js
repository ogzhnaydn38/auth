...
console.log("Starting tickatme-auth-service service...");
...
const TickatmeLoginSession = require("./project-session/tickatme-login-session");

let processActive = false;
let expressServer = null;

const start = async () => {
  process.title = "node-tickatme-auth-service";
  try {
  ...
  try {
    ...
    await setRedisData("tickatme-auth-service:start", today.toUTCString());
    console.log(
      "Service start datetime is written to Redis key tickatme-auth-service:start",
      today.toUTCString(),
    );
  } catch (err) {
  ...
  console.log(
    "tickatme-auth-service is listening HTTP/REST port " +
  ...
  // init login database
  const loginSession = new TickatmeLoginSession();
...
/* starting the server tickatme-auth-service .... */
start();
